...
Page({
  ...
  fromSubmit: function (e) {
    let that = this;
    console.log(that.data.imgSuccess)
    console.log("我的世界")

    let master = e.detail.value;
    master.imageUrl = mulImage.join(",");
    if (!master.caseCity.length) {
      master.caseCity = that.data.showRegionStr
    }
    if (!master.caseTime.length) {
      master.caseCity = that.data.date
    }
    master.caseId = id
    console.log(master);
    ...
    WXAPI.modifyTechnologyCase(master).then(res => {
      if (res.code == 200) {
        wx.hideLoading();
        wx.navigateBack({})
      } else {
  ...
  deleteCase() {
    let data = {
      technologyCaseId: Number(id)
    }
    WXAPI.deleteTechnologyCase(data).then(res => {
      if (res.code == 200) {
        wx.showToast({
          icon: "none",
          title: "删除成功",
  ...
  // 获取大师案例信息
  getTechnologyCaseInfo(id) {
    let data = {
      caseId: id
    }
    WXAPI.getTechnologyCaseInfo(data).then(res => {
      if (res.code == 200) {
        res.data.caseImageUrl = res.data.caseImageUrl.split(',')
        mulImage = res.data.caseImageUrl;
        this.setData({
          upload_picture_list: res.data.caseImageUrl,
          caseDetail: res.data,
          date: res.data.caseTime,
          showRegionStr: res.data.caseCity
        })
      }
    })
  },
